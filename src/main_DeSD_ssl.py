...
def train_DeSD(args):
    ...
    shutil.copyfile(
        "main_DeSD_ssl.py", os.path.join(args.output_dir, "code", "main_DeSD_ssl.py")
    )
    ...
    teacher = utils.MultiCropWrapper_v5_tea(
        teacher, [DINOHead(2048, args.out_dim, args.use_bn_in_head)]
    )
...
def train_one_epoch(
    ...
    for images in data_loader:
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            loss, loss_ssl, self_loss_1, self_loss_2, self_loss_3 = desd_loss(
        ...
        if now_it % 100 == 0:
            print(
                "[{}/{}]\t[{}/{}]\ttotal_loss:{}\tloss_ssl:{}\tloss_self:{}\t{}\t{}\tlr:{}\tw:{}".format(
                    epoch,
                    args.epochs,
                    now_it,
                    len(data_loader),
                    round(loss.item(), 4),
                    round(loss_ssl.item(), 4),
                    round(self_loss_1.item(), 4),
                    round(self_loss_2.item(), 4),
                    round(self_loss_3.item(), 4),
                    round(optimizer.param_groups[0]["lr"], 6),
                    round(optimizer.param_groups[0]["weight_decay"], 4),
                )
            )
...
class DeSDLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        teacher_out_self_4 = (
            F.softmax((teacher_output[-1] - self.center) / temp, dim=-1)
            .detach()
            .chunk(2)
        )
        ...
        for iq in range(len(teacher_out_self_4)):
            for v in range(len(student_out_4)):
                if v == iq:
                    ...
                    continue
                loss_1 = torch.sum(
                    -teacher_out_self_4[iq] * F.log_softmax(student_out_1[v], dim=-1),
                    dim=-1,
                ).mean()
                loss_2 = torch.sum(
                    -teacher_out_self_4[iq] * F.log_softmax(student_out_2[v], dim=-1),
                    dim=-1,
                ).mean()
                loss_3 = torch.sum(
                    -teacher_out_self_4[iq] * F.log_softmax(student_out_3[v], dim=-1),
                    dim=-1,
                ).mean()
                loss_4 = torch.sum(
                    -teacher_out_self_4[iq] * F.log_softmax(student_out_4[v], dim=-1),
                    dim=-1,
                ).mean()
